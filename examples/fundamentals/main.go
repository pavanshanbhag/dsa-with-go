...
func main() {
	...
	fundamentals.DemoComplexityDifferences()
...
func demoFibonacciComparison() {
	...
	// Naive approach
	start := time.Now()
	result1 := fundamentals.Fibonacci(n)
	naiveTime := time.Since(start)

	// Memoized approach
	start = time.Now()
	result2 := fundamentals.FibonacciMemo(n)
...
func demoSearchScaling() {
	...
	for _, size := range sizes {
		arr := fundamentals.GenerateArray(size)
		target := size - 1 // Worst case for linear search

		// Linear search timing
		start := time.Now()
		fundamentals.LinearSearch(arr, target)
		linearTime := time.Since(start)

		// Binary search timing
		start = time.Now()
		fundamentals.BinarySearch(arr, target)
